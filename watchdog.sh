...
while true; do
    if ! pgrep -f "gunicorn.*courtview|courtview.py" >/dev/null 2>&1; then
        ...
        cd "$CV_DIR" && nohup gunicorn --workers 1 --worker-class gthread --threads 4 --bind 0.0.0.0:8766 --timeout 60 --log-level warning courtview:app >> "$CV_DIR/courtview.log" 2>&1 &
    fi
    sleep 60
done

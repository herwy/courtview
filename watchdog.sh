...
while true; do
    if ! pgrep -f "gunicorn.*courtview|courtview.py" >/dev/null 2>&1; then
        ...
        cd "$CV_DIR" && gunicorn --daemon --workers 1 --worker-class gthread --threads 4 --bind 127.0.0.1:8766 --timeout 60 --log-level warning --error-logfile "$CV_DIR/courtview.log" courtview:app
    fi
    sleep 60
done
